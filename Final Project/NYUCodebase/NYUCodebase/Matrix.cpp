...
Vector4::Vector4(float in_x, float in_y, float in_z) {
	x = in_x;
	y = in_y;
	z = in_z;
	a = 1.0f;
...
Vector4 Matrix::operator * (const Vector4& v) const{
	Vector4 result;
	result.x = m[0][0] * v.x + m[1][0] * v.y + m[2][0] * v.z + m[3][0] * v.a;
	result.y = m[0][1] * v.x + m[1][1] * v.y + m[2][1] * v.z + m[3][1] * v.a;
	result.z = m[0][2] * v.x + m[1][2] * v.y + m[2][2] * v.z + m[3][2] * v.a;
	result.a = 1.0f;
	return result;
}
...
void Matrix::SetRoll(float roll) {
    m[0][0] = (float)cos(roll);
    m[1][0] = (float)-sin(roll);
    m[0][1] = (float)sin(roll);
    m[1][1] = (float)cos(roll);
}
...
void Matrix::SetPitch(float pitch) {
    m[1][1] = (float)cos(pitch);
    m[2][1] = (float)-sin(pitch);
    m[1][2] = (float)sin(pitch);
    m[2][2] = (float)cos(pitch);
}
...
void Matrix::SetPerspectiveProjection(float fov, float aspect, float zNear, float zFar) {    
    m[0][0] = (float)1.0f/tanf(fov/2.0)/aspect;
    m[1][1] = (float)1.0f/tanf(fov/2.0);
